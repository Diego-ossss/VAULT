...
// Verified free-to-use photos (Unsplash License: free for commercial use, no
// attribution required, no watermark). Marvel intentionally has no photo here —
// it keeps the original CSS halftone/speed-line pattern instead of the real
// trademarked logo.
const CATEGORY_PHOTO = {
  nfl: "https://images.unsplash.com/photo-1571829604981-ea159f94e5ad?auto=format&fit=crop&w=1600&q=60",
  mlb: "https://images.unsplash.com/photo-1708724326817-c1c2c488e33c?auto=format&fit=crop&w=1600&q=60",
  nba: "https://images.unsplash.com/photo-1694437590805-cf944ceb41c4?auto=format&fit=crop&w=1600&q=60",
  fifa: "https://images.unsplash.com/photo-1602472097151-72eeec7a3185?auto=format&fit=crop&w=1600&q=60",
  starwars: "https://images.unsplash.com/photo-1506318137071-a8e063b4bec0?auto=format&fit=crop&w=1600&q=60",
};
...
async function compressImage(file, maxDim = 420, quality = 0.55) {
  ...
  // Prefer WebP (roughly 25-35% smaller than JPEG at equal quality).
  // Browsers that don't support WebP encoding silently return a PNG instead,
  // so detect that and fall back to JPEG in that case.
...
export default function TradingCardVault() {
  ...
  async function recompressDataUrl(dataUrl, maxDim = 420, quality = 0.55) {
    const img = await new Promise((resolve, reject) => {
      const image = new Image();
      image.onload = () => resolve(image);
      image.onerror = reject;
  ...
  async function handleRecompressAll() {
    ...
    for (const card of withImages) {
      try {
        const newImage = await recompressDataUrl(card.image);
        // Only replace it if it's actually smaller - never make things worse.
        if (newImage.length < card.image.length) {
          const updatedCard = { ...card, image: newImage };
          await apiSaveCard(updatedCard);
          setCards((prev) => prev.map((c) => (c.id === card.id ? updatedCard : c)));
        }
      } catch (e) {
        // Skip any image that fails to reprocess and keep going.
      }
      done += 1;
      setRecompressProgress(`${done}/${withImages.length}`);
    }
  ...
  const photoUrl = CATEGORY_PHOTO[activeCategory];
  const scrimAlpha = bgStrength === "bold" ? 0.5 : 0.86;
  const activePattern = photoUrl
    ? {
        image: `linear-gradient(rgba(27,29,34,${scrimAlpha}), rgba(27,29,34,${scrimAlpha})), url('${photoUrl}')`,
        size: "cover, cover",
      }
    : PATTERNS[activeCategory]
    ? PATTERNS[activeCategory](activeAccent, patternOpacity)
    : null;
    ...
    <div className="vault-root">
      ...
      <div className="toolbar">
        ...
        <div className="search-sort">
          ...
          {isAdmin && cards.some((c) => c.image) && (
            <button className="btn" onClick={handleRecompressAll} disabled={recompressing}>
              {recompressing ? `Shrinking ${recompressProgress || ""}` : "Shrink existing photos"}
            </button>
          )}
