...
// Regular Netlify Function responses cap at 6MB. Streaming the response
// instead raises that to 20MB - worth using here since the full catalog
// (with every card's photo) is the one response that can get large.
function jsonStream(obj, status = 200) {
...
export default async (req) => {
  ...
  if (req.method === "GET") {
    ...
    return jsonStream({ cards, hasPassword: !!password });
  }
