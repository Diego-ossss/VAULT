...
export default function TradingCardVault() {
  ...
  useEffect(() => {
    (async () => {
      const indexRaw = await safeGet("catalog-index");
      const ids = indexRaw ? JSON.parse(indexRaw) : [];
      const results = await Promise.all(
        ids.map(async (id) => {
          const raw = await safeGet(`card:${id}`);
          return raw ? JSON.parse(raw) : null;
        })
      );
      setCards(results.filter(Boolean));
      const pw = await safeGet("admin-password");
      setHasPassword(!!pw);
      setLoading(false);
    })();
  }, []);
  ...
  async function handleUnlock() {
    setPasswordError("");
    const stored = await safeGet("admin-password");
    if (!stored) {
      if (!passwordInput.trim()) {
        setPasswordError("Enter a password to set one.");
        return;
      }
      await window.storage.set("admin-password", passwordInput, true);
      setHasPassword(true);
      setIsAdmin(true);
  ...
  async function handleSave() {
    ...
    await window.storage.set(`card:${id}`, JSON.stringify(card), true);
    ...
    if (editingId) {
      ...
      await window.storage.set("catalog-index", JSON.stringify(ids), true);
    }
    setCards(nextCards);
    setSaving(false);
    setShowForm(false);
  ...
  async function handleDelete(id) {
    await window.storage.delete(`card:${id}`, true);
    const nextCards = cards.filter((c) => c.id !== id);
    setCards(nextCards);
    await window.storage.set("catalog-index", JSON.stringify(nextCards.map((c) => c.id)), true);
